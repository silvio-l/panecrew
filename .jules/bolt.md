...
## 2024-05-24 - Avoid repeated String.prototype.split("/") in tight loops
**Learning:** In scenarios like directory file filtering, using `String.prototype.split("/")` on every pattern and file path combination causes massive array allocations and performance degradation (e.g. O(N*M) string splits).
**Action:** Pre-split strings (such as `relativePath` and glob patterns) outside of the inner loop and/or use caching/memoization to reuse the split segments.
