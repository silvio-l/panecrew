...
## 2024-09-04 - Optimize string split for Git Porcelain parsing
**Learning:** Using `String.prototype.split(" -> ")` for processing git rename operations causes unnecessary intermediate array allocations, compounding the garbage collection pauses, especially after replacing `.split("\n")`.
**Action:** Replace intermediate parsing logic using `split()` with `indexOf` and `slice` to process substrings inline, which drastically reduces string slice allocations during large output parsing.
