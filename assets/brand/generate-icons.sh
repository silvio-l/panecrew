#!/usr/bin/env bash
# DORMANT (2026-08-27, VS Code extension pivot): this script was written for
# the deleted apps/desktop Tauri app — it shells out to `pnpm tauri icon` and
# rebuilds a macOS .icns bundle, neither of which exists post-pivot. Kept
# here (relocated from apps/desktop/src-tauri/icons/source/) because
# panecrew-mark.svg, the vector source of truth for the brand mark, and the
# squircle-padding logic are still useful reference for producing extension/
# marketplace/website icon assets — but the `pnpm tauri icon` step below
# needs a replacement (e.g. plain rsvg-convert/magick exports at the sizes
# VS Code's marketplace and activity-bar icons actually need) before this
