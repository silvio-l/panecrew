...
describe("gridState", () => {
  ...
  it("closePane leert nur den einen Slot und lässt andere unangetastet", () => {
    ...
    expect(next.slots[0]).toBeNull();
    expect((next.slots[1])?.paneId).toBe("pane-1");
  });
  ...
  describe("renameFileTabs", () => {
    it("rewrites File-Tab paths under a renamed folder", () => {
      const withFile = openFileTab(
        assignProjectToSlot(INITIAL_GRID_STATE, 0, "/repo/a", "pane-0", "tab-0"),
        "pane-0",
        "file-0",
      ...
      const next = renameFileTabs(withFile, "/repo/a", "/repo/a/src", "/repo/a/lib");
      const pane = next.slots[0] as Pane;
      const fileTab = pane.tabs.find((tab) => tab.kind === "file");
      expect(fileTab).toEqual({ kind: "file", tabId: "file-0", path: "/repo/a/lib/index.ts" });
    });
    ...
    it("leaves paths outside the renamed folder untouched", () => {
      ...
      const next = renameFileTabs(withFile, "/repo/a", "/repo/a/src", "/repo/a/lib");
      expect(next).toBe(withFile);
    });

    it("only touches panes belonging to the renamed project", () => {
      const withTwo = openFileTab(
        assignProjectToSlot(
          assignProjectToSlot(INITIAL_GRID_STATE, 0, "/repo/a", "pane-0", "tab-0"),
          1,
      ...
      const next = renameFileTabs(withTwo, "/repo/a", "/repo/a/src", "/repo/a/lib");
      expect(next).toBe(withTwo);
    });
  });
});
