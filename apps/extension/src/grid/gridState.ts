...
export function activeTab(pane: Pane): PaneTab {
  return pane.tabs.find((tab) => tab.tabId === pane.activeTabId) ?? pane.tabs[0];
}
...
export function moveTab(
  ...
  const moved = pane.tabs[tabIndex];
...
function withoutTab(
  ...
  const tabId = (pane.tabs[tabIndex]).tabId;
...
export function movePaneToEmptySlot(
  ...
  const nextSlots = state.slots.slice();
  nextSlots[targetSlotIndex] = nextSlots[sourceIndex];
