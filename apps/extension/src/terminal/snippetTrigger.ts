// Ported from apps/desktop/src/terminal/snippetTrigger.ts (Tauri app) — pure
// logic, no xterm.js/DOM/Tauri dependency there either. The only change here
// is `BufferPosition`: the desktop app imports it from its own
// `terminal/suggestion.ts` (a much larger, Tauri-app-specific module that is
// out of scope for this port), so it's redeclared locally instead of pulling
// that whole module in — same shape, same meaning (absolute buffer
// coordinates, `baseY + cursorY`).
//
...
/** Position in the absolute buffer coordinate system (`baseY + cursorY`). */
