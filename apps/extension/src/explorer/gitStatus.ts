...
export function parsePorcelain(output: string, repoRoot: string): Map<string, GitFileStatus> {
  ...
  // ⚡ Performance optimization: Avoid large array allocations from .split("\n")
  // and .split(" -> ") by processing lines and strings iteratively.
  let startIndex = 0;
  while (startIndex < output.length) {
    ...
    // ⚡ Performance optimization: Avoid .split() on strings
