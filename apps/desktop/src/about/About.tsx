 ...
 *
 * Verhältnis zum Splash: gleiche Familie, kein Nachbau. Übernommen sind die
 * Gehrungsfuge der Marke als angeschnittene Ecke, das Glasrezept der Fußleiste
 * (--pc-glass-*) und deren Kolophon-Typografie. Anders als der Splash ist das
 * hier ein NORMALES App-Fenster: es lädt theme.css und folgt Dark wie Light,
 * statt eine fest dunkle Markenbühne zu sein.
 *
 * Der Anschnitt sitzt hier nicht an der Fensterkante (das Fenster hat eine
 * native Titelleiste samt Ampeln), sondern an der Bühne im Kopfbereich — die
 * Marke schneidet in die eigene Fläche, nicht in die Silhouette des Systems.
 *
...
// Geometrie des Anschnitts. Die Steigung ist die der Gehrungsfuge im Markenmal
// (325:405 im Emblem, 152:122 im Splash) — 100:80 trifft sie auf 0,3 % genau.
// Feste Pixel sind hier zulässig, weil das Fenster nicht größenveränderlich ist.
const MITRE_WIDTH = 100;
const MITRE_HEIGHT = 80;
const STAGE_CLIP = `polygon(0 0, calc(100% - ${String(MITRE_WIDTH)}px) 0, 100% ${String(MITRE_HEIGHT)}px, 100% 100%, 0 100%)`;
...
export function About() {
  ...
  return (
    <div className="relative flex h-full flex-col overflow-hidden bg-(--pc-app-background) text-(--pc-foreground)">
      <header
        ...
        // pt-13: darüber liegen die nativen Ampeln der Overlay-Titelleiste.
        className="flex shrink-0 flex-col items-center gap-2.5 px-7 pb-7 pt-13"
      >
        <BrandMark />
      ...
      {/* Ungeschnittenes Geschwister statt Kind der Bühne: im geclippten
          Element bliebe von der Linie nur die innere Hälfte stehen. */}
      <svg
        width={MITRE_WIDTH}
        height={MITRE_HEIGHT}
        viewBox={`0 0 ${String(MITRE_WIDTH)} ${String(MITRE_HEIGHT)}`}
        aria-hidden="true"
        className="pointer-events-none absolute right-0 top-0"
      >
        <path
          d={`M0 0 L${String(MITRE_WIDTH)} ${String(MITRE_HEIGHT)}`}
          fill="none"
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto px-7 pb-14 pt-5">
        <p className="text-(length:--pc-chrome-fontSize) leading-relaxed text-(--pc-descriptionForeground)">
          ...
          ein Raster aus lebenden Panes, dazu ein Datei-Explorer, der immer der
          gerade fokussierten Pane folgt.
        </p>
