...
**Planning history lives in `docs/decisions.md`** (rationale, rejected alternatives, full requirements, including the 2026-08-27 Tauri→VS-Code-extension pivot decision itself) — read it before making architectural/scope decisions. The pre-pivot `.scratch/panecrew-v0.1-spec/` and `.scratch/panecrew-v0.1/` wayfinder maps (the old Tauri desktop-app build sequence) were removed in the 2026-08-27 `.scratch` cleanup once their content was confirmed fully superseded by the actual `apps/extension` implementation and by `docs/decisions.md`'s own pivot section — there is no current wayfinder map. `.scratch/` now holds only live, still-relevant feature specs/tickets per `docs/agents/issue-tracker.md`; treat it as the active backlog, not a historical archive.
